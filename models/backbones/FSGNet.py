...
class FSGNet_noGRM(nn.Module):
    def __init__(self, channel, n_classes, base_c, depths, kernel_size):
        super(FSGNet_noGRM, self).__init__()
    ...
    def forward(self, x):
        ...
        x3_up = self.up_residual_conv3(x4)

        _, _, h, w = x2_down.size()
        x3_up = F.interpolate(x3_up, size=(h, w), mode='bilinear', align_corners=True)
        x2_up = self.up_residual_conv2(x3_up)

        _, _, h, w = x1_down.size()
        x2_up = F.interpolate(x2_up, size=(h, w), mode='bilinear', align_corners=True)
        x1_up = self.up_residual_conv1(x2_up)
